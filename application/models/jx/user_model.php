 ...
 * 
 * $Id: user_model.php,v 1.14 2013-03-06 00:34:30 junior Exp $
 * 
 */

class User_model extends JX_Model
{
	protected $_revision	=	'$Id: user_model.php,v 1.14 2013-03-06 00:34:30 junior Exp $';
...
//echo "user->id=".$user_profile['id'].'<br/>';
		...
		if ( $user_profile )
		{
			...
			if ( array_key_exists( 'picture', $user_profile ) )
			{
				...
				elseif ( is_array( $user_profile[ 'picture' ] ) )
				{
					$foto		=	$user_profile[ 'picture' ]['data']['url'];
				}
				else
			 ...
			 */
			$nova_pessoa					=	FALSE;
			// Verificamos se já existe um usuário com o e-mail enviado.
//			if ( $email )
//			{
//				// Se sim, buscamos este ID para que seja atualizado.
//				$pessoa_data		=	$this->get_one_by_where( "email = {$email}" );
//				$pessoa_id		=	$pessoa_data->id;
//			}
			if ( $pessoa_id )
			{
				$pessoa_data				=	$this->pessoa->get_one_by_id( $pessoa_id );
			}
			else
			{
				$nova_pessoa				=	TRUE;
				$pessoa_data				=	new stdClass();
			}

			if ( !$nova_pessoa ) // Pessoa já existe.
			{
				if ( !$pessoa_data->data_hora_inscricao )
				{
					$pessoa_data->data_hora_inscricao	=	'CURRENT_TIMESTAMP';
				}
				if ( !$pessoa_data->data_hora_ultimo_chute )
				{
					$pessoa_data->data_hora_ultimo_chute	=	'CURRENT_TIMESTAMP';
				}
				$pessoa_data->data_hora_ultima_atualizacao	=	'CURRENT_TIMESTAMP';

				$pessoa_data->email			=	$email; // UK
				$pessoa_data->nome			=	$nome;
				$pessoa_data->sobrenome			=	$sobrenome;
				$pessoa_data->aniversario		=	$aniversario;
				$pessoa_data->sexo			=	$sexo;
				$pessoa_data->fone_1			=	'';
				$pessoa_data->fone_2			=	'';
				$pessoa_data->imagem_facebook		=	( $foto ) ? $foto : 'http://profile.ak.fbcdn.net/static-ak/rsrc.php/v2/yo/r/UlIqmHJn-SK.gif';
			...
			else
			{
				...
				$pessoa_data->id			=	NULL;
				$pessoa_data->email			=	( !$email ) ? $pessoa_data->facebook_id : $email; // UK
				$pessoa_data->nome			=	$nome;
				$pessoa_data->sobrenome			=	$sobrenome;
				$pessoa_data->aniversario		=	$aniversario;
				$pessoa_data->sexo			=	$sexo;
			...
			{
				log_message( 'debug', "falha de update de FACEBOOK." );
echo '<pre>';
