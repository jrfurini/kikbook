 ...
 * 
 * $Id: rodada_fase_model.php,v 1.21 2013-04-14 12:58:49 junior Exp $
 * 
 */

class Rodada_fase_model extends JX_Model
{
	protected $_revision		=	'$Id: rodada_fase_model.php,v 1.21 2013-04-14 12:58:49 junior Exp $';
	...
	public function get_select_for_index()
	{
		return	"
			...
			,concat( rodada_fase.cod, ' (', date_format( rodada_fase.data_inicio, '%e/%m' ), ' até ', date_format( rodada_fase.data_fim, '%e/%m/%Y' ), ')' )		AS	title_short
...
//			,rodada_fase.data_inicio						AS	when_field
		...
		else
		{
			...
			foreach( $this->select_all( $where )->result_object() as $rodada )
			{
				...
				foreach( $query_jogos->result_object() as $data_jogos )
				{
					...
					{
						if ( $rodada->data_inicio > date( '2013-05-20 00:00:00' ) )
						{
							$rodada->peso_kik	=	5 / $data_jogos->qtde_jogos; // 5 é o max ou min por jogo.
						}
						else
						{
	...
	{
		if ( !$campeonato_versao_id ) // Se não foi informado o campeonato ou se ele veio nulo, forçamos um valor para a query retornar nada.
		{
		...
		if ( $data_hora )
		{
			$rodada		=	$this->get_one_by_where( "/*get_rodada_anterior $tipo_fase*/	rodada_fase.campeonato_versao_id = {$campeonato_versao_id}
												...
												and	rodada_fase.data_inicio in	(
																	...
																	and	( ( rod2.data_inicio 	  > adddate( date( '{$data_hora}' ), interval -30 day )
																	and	    '{$tipo_fase}'       in ( 'S', 'F' )
																	          )
																	or        '{$tipo_fase}'     not in ( 'S', 'F' )
																	        )
																	)
									"
									);
	...
	public function get_rodada_ultima( $campeonato_versao_id )
	{
		$rodada		=	$this->get_one_by_where( "/*get_rodada_proxima*/	rodada_fase.campeonato_versao_id = {$campeonato_versao_id}
											and	rodada_fase.data_inicio in	(
																select	max( rod2.data_inicio )
