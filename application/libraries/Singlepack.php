 ...
 * 
 * $Id: Singlepack.php,v 1.30 2013-04-14 12:32:04 junior Exp $
 * 
 */
...
class Singlepack
{
	...
	public function initialize( $config = array() )
	{
		if ( !$this->initalized )
		{
			...
			/**
			 * Se é um robot entrando, damos acesso sem o facebook sempre.
			 */
			if ( $this->get_sessao( 'robot_mode' ) == 'ON' )
			{
				$this->CI->config->set_item( 'facebook_login', FALSE );
			}
			
			/**
			 * Registra se estamos ou não num canvas do Facebook
			 */
			if ( $this->get_sessao( 'inside_facebook' ) === TRUE )
			{
				$this->inside_facebook		=	TRUE;
			}
			else
			{
				$this->inside_facebook		=	FALSE;
			}
	...
	*/		{
	...
	{
		log_message( 'debug', "facebook_login Single Pack.set_facebook_access_token($access_token)" );
	...
	{
		log_message( 'debug', "Single Pack.facebook_get_user_data(START)" );
		if ( $this->CI->config->item( 'facebook_login' ) == TRUE )
		{
			/*
			 * Para conseguir entender o funcionamento desta parte, observe também o home.php o "facebook_login" e "do_login". Eles tratam
			 * a montagem das variáveis access_token antes de chegar aqui.
			 */
			
			$this->facebook_access_token			=	json_decode( $this->get_sessao( 'facebook_access_token' ), TRUE );
			log_message( 'debug', "facebook_login Single Pack.facebook_get_user_data.access_token($this->facebook_access_token) (".session_id().")" );
			
			if ( $this->facebook_access_token )
			{
				log_message( 'debug', "....Tem this->facebook_access_token" );
				...
				else
				{
					...
					try	{
						// FQL
							// Retorna EU e Amigos instalados.
							$user_and_friends_fql	=	'SELECT me(), uid, email, username, name, first_name, last_name, pic, pic_square, locale, birthday_date, sex FROM user WHERE is_app_user AND ( uid IN (SELECT uid2 FROM friend WHERE uid1 = me() ) OR uid = me() )';

							// Retorna os grupos que EU faço parte.
							$user_groups_fql	=	'SELECT gid, name, description, privacy, icon, pic, email, update_time, website FROM group WHERE gid in ( SELECT gid FROM group_member WHERE uid = me() )';
							$user_groups_adm_fql	=	'SELECT gid, administrator, bookmark_order, unread, positions FROM group_member WHERE uid = me()';

							// Convites para jogar - NÃO FUNCIONOU AINDA.
							$apprequest_fql		=	'SELECT created_time, data, message, recipient_uid, request_id, sender_uid FROM apprequest WHERE app_id = 217628675010321 and recipient_uid = me()';
							
							// Pega as informações do usuário atual (EU) e dos amigos que tem o aplicativo instalado.
							$ret_obj	=	$this->facebook->api	(
													array	(
														 'method' => 'fql.query'
														,'query' => $user_and_friends_fql
														)
													);

							$this->facebook_user_profile	=	array();
							$this->facebook_friends		=	array();
							foreach( $ret_obj as $key => $user_vals )
							{
								if ( $user_vals['me'] == $user_vals['uid'] ) // EU
								{
									$this->facebook_user_profile[ 'email' ]			=	$user_vals[ 'email' ];
									$this->facebook_user_profile[ 'name' ]			=	$user_vals[ 'name' ];
									$this->facebook_user_profile[ 'first_name' ]		=	$user_vals[ 'first_name' ];
									$this->facebook_user_profile[ 'last_name' ]		=	$user_vals[ 'last_name' ];
									$this->facebook_user_profile[ 'gender' ]		=	$user_vals[ 'sex' ];
									$this->facebook_user_profile[ 'username' ]		=	$user_vals[ 'username' ];
									$this->facebook_user_profile[ 'id' ]			=	$user_vals[ 'uid' ];
									$this->facebook_user_profile[ 'locale' ]		=	$user_vals[ 'locale' ];
									$this->facebook_user_profile[ 'birthday' ]		=	$user_vals[ 'birthday_date' ];
									$this->facebook_user_profile[ 'favorite_teams' ]	=	array();
									$this->facebook_user_profile[ 'picture' ]		=	$user_vals[ 'pic_square' ]; // ou pic
								}
								else // Amigos
								{
									$ar_user						=	array();
									$ar_user[ 'name' ]					=	$user_vals[ 'name' ];
									$ar_user[ 'first_name' ]				=	$user_vals[ 'first_name' ];
									$ar_user[ 'last_name' ]					=	$user_vals[ 'last_name' ];
									$ar_user[ 'gender' ]					=	$user_vals[ 'sex' ];
									$ar_user[ 'username' ]					=	$user_vals[ 'username' ];
									$ar_user[ 'id' ]					=	$user_vals[ 'uid' ];
									$ar_user[ 'locale' ]					=	$user_vals[ 'locale' ];
									$ar_user[ 'birthday' ]					=	$user_vals[ 'birthday_date' ];
									$ar_user[ 'favorite_teams' ]				=	array();
									$ar_user[ 'picture' ]					=	$user_vals[ 'pic' ]; // ou pic_square
									$ar_user[ 'installed' ]					=	true;
									$this->facebook_friends[]				=	$ar_user;
								}
							}
/*
echo "EU <BR>";
...
echo "<br><br>";
*/
							// Pega os dados do grupo da pessoa.
							$ret_obj	=	$this->facebook->api	(
													array	(
														 'method' => 'fql.query'
														,'query' => $user_groups_fql
														)
													);

							$ret_obj_2	=	$this->facebook->api	(
													array	(
														 'method' => 'fql.query'
														,'query' => $user_groups_adm_fql
														)
													);

							$this->facebook_groups		=	array();
							foreach( $ret_obj as $key => $group_vals )
							{
								$ar_group			=	array();
								$ar_group[ 'id' ]		=	$group_vals[ 'gid' ];
								$ar_group[ 'name' ]		=	$group_vals[ 'name' ];
								$ar_group[ 'description' ]	=	$group_vals[ 'description' ];
								$ar_group[ 'privacy' ]		=	$group_vals[ 'privacy' ];
								$ar_group[ 'icon' ]		=	$group_vals[ 'icon' ];
								$ar_group[ 'email' ]		=	$group_vals[ 'email' ];
								$ar_group[ 'updated_time' ]	=	$group_vals[ 'update_time' ];
								$ar_group[ 'picture' ]		=	$group_vals[ 'pic' ];
								$ar_group[ 'website' ]		=	$group_vals[ 'website' ];
								// Informações do usuário
								$ar_group[ 'unread' ]		=	0;
								$ar_group[ 'bookmark_order' ]	=	$key;
								$ar_group[ 'administrator' ]	=	false;
								$ar_group[ 'positions' ]	=	array();
								foreach( $ret_obj_2 as $key_u => $user_group_vals )
								{
									if ( $user_group_vals[ 'gid' ] ==  $group_vals[ 'gid' ] )
									{
										$ar_group[ 'unread' ]		=	$user_group_vals[ 'unread' ];
										$ar_group[ 'bookmark_order' ]	=	$user_group_vals[ 'bookmark_order' ];
										$ar_group[ 'administrator' ]	=	$user_group_vals[ 'administrator' ];
										$ar_group[ 'positions' ]	=	$user_group_vals[ 'positions' ];
										break;
									}
								}
								$this->facebook_groups[ $ar_group[ 'bookmark_order' ] ]		=	$ar_group;
							}
/*
echo "GROUP NOVO <BR>";
print_r( $ret_obj );
...
print_r( $ret_obj_2 );
echo "<br><br>";
							$graph_url			=	"https://graph.facebook.com/me/groups?fields=id,name,description,privacy,icon,email,updated_time&access_token=" . $this->facebook_access_token;
							$this->facebook_groups		=	json_decode( file_get_contents( $graph_url ), TRUE );
echo "GRUPO OLD <BR>";
print_r( $this->facebook_groups );
exit;
*/
							$this->facebook_id		=	$this->facebook_user_profile[ 'id' ];
...
//return false;
		return	(  $this->CI->user->is_admin()
			...
			&&   isset( $this->methods_access[ $prg ]->access_level )
...
//echo 'FALSE<br/>';
	...
	// Inicializa a sessão para trocar de usuário do facebook
	...
	{
log_message( 'debug', "facebook_login. CLOSE_SESSION" );
...
/*			$this->set_new_facebook();
			...
			if ( $user )
			{
				...
				{
					$this->facebook_logout_url	=	str_replace( '%2Flogout', '', $this->facebook->getLogoutUrl( array( 'access_token' => $this->get_facebook_access_token(), "next" => $this->CI->config->item( 'base_url' ) . $this->CI->config->item( 'first_controller' ) ) ) );
				}
				else
				{
					$this->facebook_logout_url	=	str_replace( '%2Flogout', '', $this->facebook->getLogoutUrl( array( 'access_token' => $this->get_facebook_access_token(), "next" => $this->CI->config->item( 'base_url' ) . "/home" ) ) );
				}
...
//			return 'home/index';
	...
	public function unset_user()
	{
		...
		$this->facebook_access_token		=	NULL; // Anulando esta variável, não conseguiremos mais acessar os dados do usuário.
...
//		log_message( 'debug', "Single Pack.get_user($this->user_id).4" );
	...
	// Seta que está rodando dentro do facebook.
	public function set_inside_facebook()
	{
		$this->inside_facebook			=	TRUE;
		$this->set_sessao( 'inside_facebook', $this->inside_facebook );
	}
	public function inside_facebook()
	{
		return $this->inside_facebook;
	}
	...
	public function get_friends_id()
	{
		return $this->get_facebook_array_info( 'id', $where = array( 'installed' => TRUE ), NULL );
	}
	 ...
	 */
	public function get_facebook_array_info( $what = 'id', $where = NULL, $not_where = FALSE, $group = FALSE  )
	{
		$ar_what						=	$what;
		   ...
		   )
		{
			...
			if ( $group )
			{
				$facebook_info				=	$this->facebook_groups;
			}
			else
			{
				$facebook_info				=	$this->facebook_friends;
			}

			if ( $facebook_info )
			{
				foreach( $facebook_info as $info_key => $info_values )
				{
	...
	{
		return $this->get_facebook_array_info( 'id', $where = $where, NULL, $group = TRUE );
	}
	public function get_groups_info( $what = 'id', $where = array( 'administrator' => TRUE ) )
	{
		return $this->get_facebook_array_info( $what = $what, $where = $where, NULL, $group = TRUE );
	}
	...
	public function set_sistema()
	{
		...
		if ( isset( $this->prg_controller ) )
		{
			...
			else
			{
				...
				{
					log_message( 'debug', "......Localizou O SISTEMA" );
					   ...
					   )
					{
						...
						log_message( 'debug', ".........Localizou o controller via sistema" );
					}
				}
			}
		}
...
//				print_r( $e );
	...
	/**
	 * Deleta uma requisição de uso de aplicativo.
	 */
	public function delete_request_app( $req_id, $facebook_id = NULL )
	{
		$this->set_new_facebook();
		
		if ( $facebook_id )
		{
			$this->facebook->api( "/{$req_id}_{$facebook_id}", "DELETE");
		}
		else
		{
			$this->facebook->api( "/$req_id", "DELETE");
		}
	}
	...
	{
		$ret			=	$this->CI->session->userdata( $oque );
		if ( !$ret )
		{
			if ( isset( $_SESSION )
			&&   is_array( $_SESSION )
			&&   key_exists( $oque, $_SESSION )
			   )
			{
				$ret	=	$_SESSION[ $oque ];
			}
